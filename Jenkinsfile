    ...
    stages {
        ...
        stage('Build Docker Image') {
            steps {
                ...
                sh 'docker build -t pmhChris/damdda-ai-flask-server .'
                
                // Push the Docker image to Docker Hub.
                sh 'docker push pmhChris/damdda-ai-flask-server'
            }
        }
                ...
                sshagent (credentials: ['jenkins-ssh-credentials']) {
                    sh '''
                        ssh user@211.188.48.96 << EOF
                            ...
                            docker pull pmhChris/damdda-ai-flask-server
                            
                            # Run a new container from the pulled image, exposing it on port 5000.
                            docker run -d --name damdda-ai-flask-server -p 5000:5000 pmhChris/damdda-ai-flask-server
                        EOF
                    '''
                }
            }
